...
class TestCheckSyntax:
    ...
    def test_notebook_with_magics_is_valid(self):
        ...
        assert _check_syntax(code, "pipeline/notebooks/bronze/ingest.py") == []
...
class TestShouldRunRuff:
    def test_pipeline_lib_file_runs_ruff(self):
        assert _should_run_ruff("pipeline/pipeline_lib/storage/s3_client.py") is True

    def test_notebook_file_skips_ruff(self):
        assert _should_run_ruff("pipeline/notebooks/bronze/ingest.py") is False

    def test_deploy_script_runs_ruff(self):
        assert _should_run_ruff("pipeline/deploy/create_workflow.py") is True

    def test_non_python_file_skips_ruff(self):
        assert _should_run_ruff("pipeline/observer_config.yaml") is False

    def test_empty_path_skips_ruff(self):
        assert _should_run_ruff("") is False

    def test_windows_path_separator(self):
        assert _should_run_ruff("pipeline\\notebooks\\bronze\\ingest.py") is False
        assert _should_run_ruff("pipeline\\pipeline_lib\\foo.py") is True
...
class TestValidateFix:
    ...
    def test_valid_notebook_passes_without_ruff(self):
        ...
        result = validate_fix(code, "pipeline/notebooks/bronze/ingest.py")
        assert result.valid is True
        assert result.checks_run == ["syntax"]  # ruff pulado

    def test_pipeline_lib_without_ruff_still_validates_syntax(self):
        """Sem ruff instalado, o resultado depende so de syntax."""
        code = "def foo():\n    return 1\n"

        with patch(
            "pipeline_lib.agent.observer.validator._run_ruff",
            return_value=None,
        ):
            result = validate_fix(code, "pipeline/pipeline_lib/foo.py")
    ...
    def test_pipeline_lib_with_ruff_ok(self):
        code = "def foo():\n    return 1\n"
        with patch(
            "pipeline_lib.agent.observer.validator._run_ruff",
            return_value=(True, []),
    ...
    def test_pipeline_lib_with_ruff_errors(self):
        code = "def foo():\n    return 1\n"
        with patch(
            "pipeline_lib.agent.observer.validator._run_ruff",
...
class TestRuffSmoke:
    ...
    def test_real_ruff_on_clean_code(self):
        ...
        result = validate_fix(code, "pipeline/pipeline_lib/soma.py")
        assert result.valid is True
        assert "syntax" in result.checks_run
        assert "ruff" in result.checks_run
