...
`pipeline/pipeline_lib/agent/observer/providers/groq_provider.py`:
...
from pipeline_lib.agent.observer.providers.anthropic_provider import SYSTEM_PROMPT
...
with contextlib.suppress(ImportError):
    from pipeline_lib.agent.observer.providers import groq_provider  # noqa: F401
```
...
`pipeline/tests/test_observer/test_groq_provider.py`:

```python
import json
from pipeline_lib.agent.observer.providers.groq_provider import GroqProvider
...
```python
from pipeline_lib.agent.observer.providers import create_llm_provider
...
```python
with contextlib.suppress(ImportError):
    from pipeline_lib.agent.observer.providers import gitlab_provider  # noqa: F401
```
