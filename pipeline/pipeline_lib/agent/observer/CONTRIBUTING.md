...
pytest tests/test_observer/ --cov=pipeline_lib.agent.observer --cov-report=term-missing
```
...
```bash
ruff check pipeline_lib/agent/observer/
ruff check pipeline_lib/agent/observer/ --fix  # auto-fix
```
