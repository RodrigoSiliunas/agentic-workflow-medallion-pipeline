...
MOCK_STEP_LOGS: dict[str, list[str]] = {
    ...
    "s3": [
        "head_bucket flowertex-medallion-datalake -> NotFound",
        "create_bucket datalake (us-east-2)",
        "put_bucket_versioning + encryption AES256 + public-access block",
        "head_bucket flowertex-medallion-datalake-root -> NotFound",
        "create_bucket workspace root (us-east-2)",
        "put_bucket_policy: Databricks AWS principal allowed",
    ],
    "network": [
        "describe_vpcs filter:flowertex -> empty",
        "create_vpc 10.0.0.0/16 + IGW + public subnet",
        "create NAT Gateway (waiting available...)",
        "create 2 private subnets us-east-2a/b + route via NAT",
        "create SG with self-reference ingress",
        "POST /accounts/{id}/networks -> network_id",
    ],
    "workspace_credential": [
        "iam create_role medallion-pipeline-databricks-xaccount-role",
        "trust policy: Databricks AWS account 414351767826 + sts:ExternalId",
        "put_role_policy DatabricksEC2Policy + DatabricksS3RootPolicy",
        "POST /accounts/{id}/credentials -> credentials_id",
    ],
    "storage_configuration": [
        "POST /accounts/{id}/storage-configurations",
        "root_bucket_info: flowertex-medallion-datalake-root",
        "storage_configuration_id obtido",
    ],
    "workspace_provision": [
        "POST /accounts/{id}/workspaces (PROVISIONING)",
        "polling workspace_status...",
        "workspace RUNNING",
        "SCIM: create admin user + add to admins group",
        "POST /api/2.0/token/create -> PAT lifetime=infinite",
    ],
    "metastore_assign": [
        "GET /accounts/{id}/metastores -> discover metastore us-east-2",
        "GET /workspaces/{id}/metastore -> currently unattached",
        "PUT /accounts/{id}/workspaces/{id}/metastore (default_catalog=medallion)",
    ],
    "cluster_provision": [
        "w.clusters.list() -> none with our tag",
        "w.clusters.create node_type=m5d.large + DBR 14.3 LTS",
        "polling cluster state... PENDING -> RUNNING",
